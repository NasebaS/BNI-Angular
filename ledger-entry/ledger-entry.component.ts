...
export class LedgerEntryComponent implements OnInit {
  ...
  ngOnInit() {
    this.getLedgerList();
    this.formValidation();
    console.log("Initial Ledger Names:", this.ledgerNames);
  }
      ...
      (response: any) => {
        ...
        if (response.status == 0) {
          ...
          this.ledgerList.forEach(ledger => {
            if (!this.ledgerNames.includes(ledger.name)) { // Check duplicates
  ...
  userClicked(user) {
    ...
    if (this.selectedUsers != null) {
      this.ledgerId = this.selectedUsers.ledgerId; // Assign the ledgerId value
  ...
  async postuser() {
    ...
    if (this.newUsersForm.invalid) {
      ...
      const ledgerName = this.newUsersForm.value.ledger_name;
      if (this.ledgerNames.includes(ledgerName)) {
        Swal.fire("Error.", "Ledger entry already exists.", "error");
        return;
      }
